    ...
    with apri_db() as db:
        candidature = db.execute('''
            SELECT c.*, a.nome AS nome_azienda,
                   sc.nome AS nome_stato, sc.colore AS colore_stato
            FROM candidature c
...
def nuova():
    with apri_db() as db:
        ...
        referenti = db.execute('''
            ...
            ORDER BY r.nome
        ''').fetchall()
        stati     = db.execute('SELECT * FROM stati_candidatura WHERE attiva = 1 ORDER BY ordine').fetchall()
        ...
        if request.method == 'POST':
            ...
            if not posizione or not azienda_id or not stato_id:
                ...
                return render_template('candidature/form.html',
                                       ...
                                       stati=stati, modalita=modalita, dati=request.form)

            db.execute('''
                INSERT INTO candidature
                    (azienda_id, referente_id, posizione, stato_candidatura_id,
                     modalita_lavoro_id, versione_cv, data_candidatura, data_colloquio, url_offerta, note)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                azienda_id,
...
def modifica(id):
    with apri_db() as db:
        candidatura = db.execute('SELECT * FROM candidature WHERE id = ? AND attiva = 1', (id,)).fetchone()
        ...
        if request.method == 'POST':
            ...
            db.execute('''
                UPDATE candidature
                SET azienda_id=?, referente_id=?, posizione=?, stato_candidatura_id=?,
...
def elimina(id):
    with apri_db() as db:
        candidatura = db.execute('SELECT posizione FROM candidature WHERE id = ?', (id,)).fetchone()
        if candidatura:
