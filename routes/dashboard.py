...
def index():
    with apri_db() as db:
        ...
        # "ha risposto" = candidature uscite dai primi due stati (INVIATA, FOLLOW-UP)
        risposte = db.execute('''
            SELECT COUNT(*) FROM candidature c
            JOIN stati_candidatura sc ON c.stato_candidatura_id = sc.id
        ...
        # --- Grafico a barre: candidature per stato ---
        ...
        # --- Grafico a torta: positivo / negativo / neutro ---
        per_tipo = db.execute('''
    ...
    # preparo i dati per Chart.js come JSON
